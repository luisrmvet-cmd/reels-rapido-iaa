...
/**
 * Gerador de roteiro por TEMPLATES (sem IA paga).
 * Cada estilo tem uma estrutura narrativa diferente,
 * baseada em padrões que funcionam em Reels/TikTok.
 */
...
export function gerarRoteiro(params: ParametrosGeracao): RoteiroGerado {
...
function substituirVariaveis(texto: string, assunto: string): string {
  return texto.replace(/\{assunto\}/g, assunto.trim());
}

interface TemplateRoteiro {
  capa: string;
  cenas: Array<{ textoTela: string; narracao: string }>;
  ctaFinal: string;
  legenda: string;
  hashtags: string[];
}

const TEMPLATES_POR_ESTILO: Record<EstiloReel, TemplateRoteiro[]> = {
  venda: [
    {
  ...
  dor: [
    {
      ...
      legenda:
        "Se você se identificou com esse vídeo, comenta aqui 💬\n\nNão é frescura, é real.\n\nVamos resolver isso juntos 🙏",
